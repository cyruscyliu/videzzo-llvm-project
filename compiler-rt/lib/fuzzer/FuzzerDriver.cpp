...
void Merge(Fuzzer *F, FuzzingOptions &Options, const Vector<std::string> &Args,
  ...
  for (auto &Path : NewFiles)
    F->WriteToOutputCorpus(FileToVector(Path, Options.MaxLen), false);
...
Vector<std::string> ParseSeedInuts(const char *seed_inputs) {
  ...
  if (seed_inputs[0] == '@')
    SeedInputs = FileToString(seed_inputs + 1); // File contains list.
  else
    SeedInputs = seed_inputs; // seed_inputs contains the list.
  if (SeedInputs.empty()) {
...
Vector<std::string> ParsePreSeedInuts(const char *seed_inputs) {
  // Parse -pre_seed_inputs=@seed_inputs_file
  // Not use ParseSeedInuts because it is slow
  Vector<std::string> Files;
  if (!seed_inputs) return Files;
  ...
  // Parse SeedInputs
  std::ifstream T(seed_inputs + 1);
  while (std::getline(T, seed_input)) {
    Files.push_back(seed_input);
  }
  return Files;
}
...
int FuzzerDriver(int *argc, char ***argv, UserCallback Callback) {
  ...
  if (!Inputs->empty() && !Flags.minimize_crash_internal_step) {
    ...
    if (!IsFile(OutputCorpusDir)) {
      ...
      Options.OutputCorpusNotInteresting = RemoveLastSeparator(OutputCorpusDir) + ".not_interesting";
      ValidateDirectoryExists(Options.OutputCorpusNotInteresting, true);
    }
  }
  ...
  if (RunIndividualFiles) {
    ...
    Printf("%s: Running %zd inputs %d time(s) each.\n", ProgName->c_str(),
           Inputs->size(), Runs);
    Printf("INFO: Reading pre_seed_input if any ...\n");
    auto CorporaFiles = ReadCorpora({}, ParsePreSeedInuts(Flags.pre_seed_inputs));
    Printf("INFO: Executing pre_seed_input if any ...\n");
