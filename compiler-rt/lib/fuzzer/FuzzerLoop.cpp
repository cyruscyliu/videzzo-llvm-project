...
Fuzzer::Fuzzer(UserCallback CB, InputCorpus &Corpus, MutationDispatcher &MD,
  ...
  F = this;
  TPC.ResetMaps();
  TPC.ResetStateCounterAccumulated();
  IsMyThread = true;
...
std::string Fuzzer::WriteToOutputCorpus(const Unit &U, const bool NotInteresting) {
  ...
  if (Options.OutputCorpus.empty())
    return "";
  std::stringstream ss;
  ss << std::setw(10) << std::setfill('0') << TotalNumberOfValidRuns;
  std::string Prefix = ss.str();
  std::string Filename = Prefix + '-' + Hash(U);
  std::string Path;
  if (NotInteresting) {
    Path = DirPlusFile(Options.OutputCorpusNotInteresting, Filename);
  } else {
    Path = DirPlusFile(Options.OutputCorpus, Filename);
  }
  WriteToFile(U, Path);
  if (Options.Verbosity >= 2)
...
void Fuzzer::MutateAndTestOne() {
  ...
  for (int i = 0; i < Options.MutateDepth; i++) {
    ...
    if (NewCov) {
      ...
      // HACK: store all testcases
      WriteToOutputCorpus({CurrentUnitData, CurrentUnitData + Size}, true);
    }
